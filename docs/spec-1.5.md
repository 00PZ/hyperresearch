...
**Revision 1.5.5:** JSONL tail repair must not truncate-rewrite the valid prefix in place. Write a complete replacement then atomically replace, or append a newline to a valid final object. A failed repair leaves earlier complete events intact. 1.5.4 still holds.
...
- Dual-write / resume:
  ...
  - Concurrent writers: exclusive lock (or equivalent) around each append. Aggregators and tests treat `event_id` as the unique key; duplicate lines with the same `event_id` count as one. Under that lock, if the file’s last record is truncated (no complete JSON object, with or without a trailing newline), repair or quarantine that fragment **before** appending the saved event. A complete final JSON object that only lacks a newline stays one valid record: **append the missing newline without rewriting the prefix**. Truncated-tail repair must not open the live file in truncating `w` mode. Write a replacement file and `os.replace` (or equivalent) only after that replacement is complete, or cut the tail at a verified byte offset without rewriting valid bytes. If repair fails (disk full, kill during write), earlier complete events remain. After a successful repair, the recovered `event_id` is independently parseable exactly once.
