...
- **Spec 2.2.6 — PR #3 host/package/workflow gaps.** Production `hr-workflow drain` calls `execute_run` with the configured runtime and KnowledgeReader (FakeRuntime only `--runtime fake`). Host gaps resolve against durable snapshots and recorded memory searches. Package validator requires Spec 1 cite-check/independence binds. Files isolation is `companies/stark-industries/`. Missing `HYPERRESEARCH_SHOSHIN_VAULT` fails closed. MCP `isError` is a backend error. Envelope `put_raw_data` sends evidence bytes. Index merge parses Markdown/YAML. ingest-retry takes the company lock. `workflow.json` is atomic.
- **Spec 2.2.4 — KnowledgeReader, verified package, `hr-workflow`.** Pipeline core gains a KnowledgeReader (`none` / `files` / lazy `gbrain` extra), memory-then-gaps-then-web search, evidence snapshots with provenance, and an atomic `verified-package/` (`hyperresearch.package.v1`). Empty provenance does not count as an independent source. Company queue, GBrain publish, and Paperclip ingest live in the `hr-workflow` console script, not `hpr`.
