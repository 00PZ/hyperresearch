...
- Preserve: profile budgets, levers, provenance, untrusted-source handling, scholar/OA recovery. Claude-in-Chrome browser escalation: **unsupported** in Spec 1. Do not crash the run and do not block research. Quote-integrity stays fail-closed for unmatched ≥5-word quoted spans; the host **unquotes** those spans (drops the marks, keeps the words) before ship-check so scare-quotes cannot block a run. Do not skip the check. Do not treat unmatched quotes as source excerpts. If that unquote changes the report hash and cite-check `ok` is already true, restamp `report_hash` on the findings file (majors do not fail the bind). `blocked_on=verify` may re-enter `_ship` after the report is fixed; other blocked_on values still refuse. Abandon queued/in-progress/needs_human escalations (login wall, bot block, captcha) and continue; the failed fetch stays a skipped source.
