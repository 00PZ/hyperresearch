...
- Budget: `budget_usd=0` prevents model dispatch; exhaustion cannot `verified`; resume preserves spend. $1 budget with actual `cost_usd=0.01` must not block after the first call; remaining budget stays usable.
...
- Resume: crash between model success and first host action still executes the fetch; long draft restored byte-for-byte; structured patch restored. `run_many` timeout/uncertain → `uncertain_remote`, no second dispatch of that task_id; successful siblings cached.
