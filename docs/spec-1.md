...
- Cite-check: invalid JSON blocks; dangling mechanical citations block even with empty model findings; checker payload contains report text and source excerpts.
- Evidence manifest: fetched and reused sources in the run audit; unrelated notes excluded; mutating a selected source invalidates verification when digest text is unchanged.
- Budget: `budget_usd=0` prevents model dispatch; exhaustion cannot `verified`; resume preserves spend.
- Full graph: task payloads include upstream artifacts; configured draft count and four distinct critics; a critic finding reaches the patcher; a draft reaches synthesis.
- Resume: crash between model success and first host action still executes the fetch; long draft restored byte-for-byte; structured patch restored.
