...
- `run_many`: retry only **known-safe** failed members. Do not auto-retry `RuntimeTimeout` or other uncertain transport outcomes (provider may have accepted and billed). Persist `uncertain_remote` and do not auto-POST again without documented provider idempotency. Successful siblings stay cached. ModelRuntime must classify ambiguous non-timeout transport failures (`httpx.ReadError`, `RemoteProtocolError`, connection reset after the request was sent) as uncertain submissions, not `RuntimeFailure` that `run_many_retry` retries. Only explicitly safe failures enter the retry list.
